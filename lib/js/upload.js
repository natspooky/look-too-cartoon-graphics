...
function createImageBuffer(file) {
    ...
    e.onload = () => {
        ...
        ctx.drawImage(e, 0, 0, img.width, img.height, 0, 0, size, size)
        //console.log('working')
        img = loadImage(canvas.toDataURL("image/png"), () => {startGame(); changePage('game', document.getElementById('upload'))})
        let x = document.getElementById('testimage').src = canvas.toDataURL("image/png")
