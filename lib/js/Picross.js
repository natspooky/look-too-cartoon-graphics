...
function generateRandom() {
	...
	sol = Array( x ).fill(0).map( (_, i) => Array(y).fill(0).map( (_, j) => int(puzzleNoise(i, j) < 0.6) ) )
}
...
function drawWin() {
	...
	for (let i = 0; i < GRIDX; i++) {
		for (let j = 0; j < GRIDY; j++) {
			fill( isImage ? img.get(i, j) : ( int( puzzleNoise(i, j) < 0.6) * 255) )
...
function draw() {
	if (!started) { return 0 }
	clear()
	//mouseIsPressed && fillSquare(currentFill)
	drawGrid()
	won && drawWin(isImage)
}
...
function mousePressed() {
	if (!started) { return 0 }
	if (won) {return 0}
	fillSquare(1 + int(mouseButton === RIGHT))
	if (checkWin() && gallery) {
		// micheal code <<<<<
	}
