...
const randThresh = 0.5 // threshold for random noise
const noiseFreq = 1    // frequency of the noise, how random it is
...
function checkWin() {
	won = grid.every( (x, j) => x.every( (y, i) => 
	{
		let s = sol[i][j]
		if ( s == 1 && y == 1) {
			return true
		} else if ( s == 0 && ( y == 0 || y == 2 || y == 3) ) {
			return true
		} else {
			return false
		}
...
function startGame() {
	...
	canvas.style.height = canvas.style.width = '58vh'
	puzzleSeed = random(10000)
	noiseSeed(puzzleSeed)
	won = false
	...
	if(checkWin() && gallery) {
		tileCompleted(size, galleryIndex, document.getElementById('gallery').getAttribute('currentImg'))
		generateGalleryTile(size, document.getElementById('gallery').getAttribute('currentImg'), changePage('gallery', document.getElementById('game')))
	}
}
	...
	for (let i = 0; i < GRIDX; i++) {
		for (let j = 0; j < GRIDY; j++) {
			...
			if ( grid[i][j] == FULL ) {
				...
				if ( grid[i][j] == ERROR ) { stroke(COLS[2]) } else { stroke(COLS[1]) }
				strokeWeight(WEIGHT)
...
function fillSquare() {
	...
	if (coord != -1) {
		...
		if ( coord[0] < 0 || coord[0] > size || coord[1] < 0 || coord[1] > size || isNaN(coord[0]) || isNaN(coord[1]) ) {return -1}
